import"./assets/styles-e127b48f.js";import{n as v,a as d}from"./assets/vendor-424513b8.js";function u(e,s){localStorage.setItem(e,JSON.stringify(s))}function n(e){const s=localStorage.getItem(e);try{return JSON.parse(s)}catch{return v.Notify.failure("Oops! Something went wrong..."),s}}function k(e){const{keyword:s,category:t,page:a,limit:o,...r}=e,l=Object.keys(r).length,i=new URLSearchParams({page:a,limit:o});if(s&&i.set("keyword",s),t&&i.set("category",t),l){const[b,h]=Object.entries(r)[0];i.set(b,h)}return i}function P(e,s,t){const a=t.findIndex(o=>o._id===s._id);a!==-1?t.splice(a,1):t.push(s),u(e,t)}const f="basket",$=n(f)??[],w=document.querySelector(".product-list");async function L(e){if(!e.target.closest(".btn-basket"))return;const t=e.target.closest(".resp-item").dataset.id,o=n("main products").filter(({_id:r})=>t===r)[0];P(f,o,$)}w.addEventListener("click",L);d.defaults.baseURL="https://food-boutique.b.goit.study/api";const S=async()=>{const{data:e}=await d.get("/products/categories");return e},p=async e=>{const s=k(e),{data:t}=await d.get("/products/",{params:s});return t},C=async e=>{const{data:s}=await d.get(`/products/popular?limit=${e}`);return s},A=async()=>{const{data:e}=await d.get("/products/discount");return e},y="/Code-Storm/assets/sprite-f073391d.svg",E={popularList:document.querySelector(".popular-list")};function O(e){const s=e.map(({name:t,img:a,category:o,size:r,popularity:l,_id:i})=>`<li class="popular-item" data-id="${i}">
      ...
      </li>`);j.discountList.insertAdjacentHTML("beforeend",s.join(""))}A().then(e=>B(e));const M=document.querySelector(".product-list"),q="main products";function g({results:e}){u(q,e);const s=e.map(t=>{let a=x(t.category),o=T(t.price);return`<li class="resp-item" data-id=${t._id} data-info="${t}">
        <div class="img" >
